...
/**
 * Check if immersive-ar is available at all (does NOT require depth sensing).
 */
export async function checkARSupport(): Promise<{ supported: boolean; reason?: string }> {
  if (typeof navigator === 'undefined' || !('xr' in navigator)) {
    return { supported: false, reason: 'WebXR not available — use Safari on iPhone' }
  }
  try {
    const ok = await (navigator as any).xr.isSessionSupported('immersive-ar')
    return ok
      ? { supported: true }
      : { supported: false, reason: 'Immersive AR not supported on this device/browser' }
  } catch (e) {
...
/**
 * Explicitly request camera permission via getUserMedia before WebXR.
 * This makes the iOS permission dialog appear with a clear context,
 * and avoids silent failures if the user has previously denied camera.
 */
 ...
 * - depth-sensing is OPTIONAL: session starts even without it (shows AR camera feed)
 * - Canvas is attached to the DOM so the camera feed actually renders
 */
export async function startXRSession(options: XRSessionOptions): Promise<XRStartResult> {
  ...
  // Remove any leftover canvas from a previous session
  if (xrCanvas) { xrCanvas.remove(); xrCanvas = null }

  const session = await (navigator as any).xr.requestSession('immersive-ar', {
    requiredFeatures: ['local'],          // only 'local' is required; most devices support it
    optionalFeatures: [
      'local-floor',
      'depth-sensing',                    // LiDAR — optional so session starts regardless
      'dom-overlay',
    ],
    depthSensing: {
  ...
  // Create canvas, attach to DOM — required for iOS to show the camera feed
  ...
  // Try local-floor first, fall back to local
  try {
  ...
  // Check if depth sensing actually activated (it's optional)
  const hasDepthSensing = !!(session as any).depthUsage

  session.addEventListener('end', () => {
    xrCanvas?.remove(); xrCanvas = null
    options.onEnd()
  })
...
function onXRFrame(time: number, frame: XRFrameWithDepth, options: XRSessionOptions): void {
  ...
  for (const view of pose.views) {
    // getDepthInformation may not exist if depth-sensing didn't activate
